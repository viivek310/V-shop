    ...
    return (
        <>
            {<ToastContainer
                ...
                theme="light"
            />}
            <div className='min-h-[100svh] px-20 '>

                <div className="select page bg-gray-200 px-36 py-5 pb-10 rounded-xl">
                    <div className="grid gap-4 w-[100%] justify-items-center">
                        <div className='w-[80%] relative h-[70svh]'>
                        ...
                        <div className="grid grid-cols-5 gap-4">
                            {images.map((img, ind) => {
                                return <div className='h-36' key={ind} onClick={() => setindex(ind)}>
                    ...
                    <div className='py-10 px-5 font-bold text-2xl'>{product?.productName}</div>
                    <div className='px-5 text-2xl flex gap-5'>
                        <span className='text-green-500'>{product?.productDiscount}% off</span>
                        <span className='line-through text-slate-500'>₹{product?.productPrice}</span>
                        <span className='font-bold'>₹{product?.productDisPrice}</span>
                    </div>
                    <div className='buttons flex gap-5 py-12 justify-center text-xl'>
                        <button onClick={handleclick} className='bg-purple-600 hover:bg-purple-800 w-1/2  py-4 text-white p-2 rounded-full font-bold flex justify-center items-center gap-3'><span><MdOutlineShoppingCart /></span>Add to cart</button>
                        <button className='bg-orange-600 hover:bg-orange-800 w-1/2 py-4 text-white p-2 rounded-full font-bold flex justify-center items-center gap-3'><span><GiShoppingBag /></span>Buy Now</button>

                    </div>
                    <div>
                        <h3 className='font-bold text-2xl py-5'>Details</h3>
                    ...
                    <div className='suggestion'>
                        <h3 className='font-bold text-2xl py-5'>Products from the same category</h3>
                                ...
                                return <div key={ind} onClick={()=>changeRouter(sug.productID)} className='w-48 h-64  shadow-lg rounded-lg overflow-hidden'>
                                    ...
                                    <div className="productdes p-2 text-sm bg-violet-100 h-full">
                                        ...
                                        <div className="price flex gap-2 py-2">
                                            <span className='font-bold'>{sug.productDisPrice}</span>
                                            <span className='line-through'>{sug.productPrice}</span>
